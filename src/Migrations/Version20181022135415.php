 ...
 */
final class Version20181022135415 extends AbstractMigration
{
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__run AS SELECT id, cluster_id, program_id, start FROM run');
        $this->addSql('DROP TABLE run');
        $this->addSql('CREATE TABLE run (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cluster_id INTEGER NOT NULL, program_id INTEGER NOT NULL, start DATETIME NOT NULL, label VARCHAR(255) DEFAULT NULL, description CLOB DEFAULT NULL, CONSTRAINT FK_5076A4C0C36A3328 FOREIGN KEY (cluster_id) REFERENCES cluster (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_5076A4C03EB8070A FOREIGN KEY (program_id) REFERENCES program (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO run (id, cluster_id, program_id, start) SELECT id, cluster_id, program_id, start FROM __temp__run');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__run AS SELECT id, cluster_id, program_id, start FROM run');
        $this->addSql('DROP TABLE run');
        $this->addSql('CREATE TABLE run (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cluster_id INTEGER NOT NULL, program_id INTEGER NOT NULL, start DATETIME NOT NULL)');
        $this->addSql('INSERT INTO run (id, cluster_id, program_id, start) SELECT id, cluster_id, program_id, start FROM __temp__run');
        $this->addSql('DROP TABLE __temp__run');
        $this->addSql('CREATE INDEX IDX_5076A4C0C36A3328 ON run (cluster_id)');
        $this->addSql('CREATE INDEX IDX_5076A4C03EB8070A ON run (program_id)');
