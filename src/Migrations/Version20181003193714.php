 ...
 */
final class Version20181003193714 extends AbstractMigration
{
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__cluster AS SELECT id, label, description FROM cluster');
        $this->addSql('DROP TABLE cluster');
        $this->addSql('CREATE TABLE cluster (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, label INTEGER NOT NULL, description VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO cluster (id, label, description) SELECT id, label, description FROM __temp__cluster');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__cluster AS SELECT id, label, description FROM cluster');
        $this->addSql('DROP TABLE cluster');
        $this->addSql('CREATE TABLE cluster (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, label INTEGER NOT NULL, description VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO cluster (id, label, description) SELECT id, label, description FROM __temp__cluster');
        $this->addSql('DROP TABLE __temp__cluster');
        $this->addSql('DROP INDEX IDX_BF3BAD1BC36A3328');
        $this->addSql('CREATE TEMPORARY TABLE __temp__luminaire AS SELECT id, cluster_id, serial, address FROM luminaire');
