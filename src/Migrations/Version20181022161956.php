 ...
 */
final class Version20181022161956 extends AbstractMigration
{
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__run AS SELECT id, cluster_id, program_id, start, label, description FROM run');
        ...
        $this->addSql('INSERT INTO run (id, cluster_id, program_id, start, label, description) SELECT id, cluster_id, program_id, start, label, description FROM __temp__run');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__run AS SELECT id, cluster_id, program_id, start, label, description FROM run');
        $this->addSql('DROP TABLE run');
        $this->addSql('CREATE TABLE run (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cluster_id INTEGER NOT NULL, program_id INTEGER NOT NULL, start DATETIME NOT NULL, label VARCHAR(255) DEFAULT NULL, description CLOB DEFAULT NULL)');
        $this->addSql('INSERT INTO run (id, cluster_id, program_id, start, label, description) SELECT id, cluster_id, program_id, start, label, description FROM __temp__run');
        $this->addSql('DROP TABLE __temp__run');
        $this->addSql('CREATE INDEX IDX_5076A4C0C36A3328 ON run (cluster_id)');
        $this->addSql('CREATE INDEX IDX_5076A4C03EB8070A ON run (program_id)');
